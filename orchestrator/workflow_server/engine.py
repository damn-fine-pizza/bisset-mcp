...
class WorkflowEngine:
    ...
    def _feature_file_path(self, task_id: str) -> str | None:
        cfg = self._get_bdd_config()
        if not cfg['project_path']:
            return None
        return os.path.join(cfg['project_path'], cfg['features_dir'], f'{task_id}.feature')
    ...
    def add_task(self, task_id: str, title: str, description: str = '', acceptance_criteria: str = '') -> dict:
        ...
        logger.info('add_task tid=%s title=%r feature_path=%s', task_id, title, feature_path)
        return {'ok': True, 'task_id': task_id, 'feature_path': feature_path}
